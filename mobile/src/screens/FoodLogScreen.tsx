...
export default function FoodLogScreen() {
  ...
  useEffect(() => {
    ...
    showTip('tip_log_share', { title: t('tour.tipLogShareTitle'), body: t('tour.tipLogShareBody') })
  ...
  useEffect(() => {
    ...
    showTip('tip_log_scan', { title: t('tour.tipLogScanTitle'), body: t('tour.tipLogScanBody') })
  }, [seenFeatureTips, showTip, t])
    ...
    <Screen contentContainerStyle={{ paddingHorizontal: 18, paddingTop: 8 }}>
        ...
        <View style={styles.headerActions}>
          <TourTarget id="tip_log_scan">
            <Pressable onPress={() => setShowScanner(true)} style={[styles.headerBtn, { backgroundColor: theme.colors.backgroundElevated }]}>
              ...
              <Text style={{ fontSize: 12, fontWeight: '600', color: theme.colors.textSecondary }}>{t('log.scan')}</Text>
            </Pressable>
          </TourTarget>
          <TourTarget id="tip_log_share">
            <Pressable onPress={handleShareLog} style={[styles.headerBtn, { backgroundColor: theme.colors.backgroundElevated }]}>
              ...
              <Text style={{ fontSize: 12, fontWeight: '600', color: theme.colors.textSecondary }}>{t('log.share')}</Text>
            </Pressable>
          </TourTarget>
        </View>
      </View>
      ...
      <Card style={styles.summaryCard}>
        <View style={styles.summaryCell}>
          <Text style={[styles.summaryLabel, { color: theme.colors.textTertiary }]}>{t('log.eaten')}</Text>
        ...
        {totalBurned > 0 && (
          <>
            ...
            <View style={styles.summaryCell}>
              <Text style={[styles.summaryLabel, { color: theme.colors.textTertiary }]}>{t('log.burned')}</Text>
              <Text style={[styles.summaryValue, { color: theme.colors.calories }]}>{Math.round(totalBurned)}</Text>
            </View>
            <Text style={{ color: theme.colors.textTertiary, fontSize: 16 }}>=</Text>
            <View style={styles.summaryCell}>
              <Text style={[styles.summaryLabel, { color: theme.colors.textTertiary }]}>{t('log.net')}</Text>
              ...
              <View style={styles.mealHeader}>
                <View style={styles.mealLeft}>
                  ...
                  <View>
                    ...
                    {items.length === 0 ? (
                      <Text style={[styles.mealMeta, { color: theme.colors.textTertiary }]}>{t('log.noItemsYet')}</Text>
                    ) : (
                      <Text style={[styles.mealMeta, { color: theme.colors.textTertiary }]}>
                        {t('log.mealSummary', { calories: Math.round(mealCals), protein: formatMass(mealProt, system) })}
                      </Text>
                    )}
                  </View>
                </View>
                <Pressable onPress={() => handleAddClick(meal)} style={[styles.addPill, { backgroundColor: theme.colors.accentSoft }]}>
                  ...
                  <Text style={{ fontSize: 12, fontWeight: '600', color: theme.colors.accent }}>{t('common.add')}</Text>
                </Pressable>
              </View>
                        ...
                        <View style={styles.itemNameRow}>
                          ...
                          {isMealEntry && (
                            <View style={[styles.mealTag, { backgroundColor: theme.colors.accentSoft }]}>
                              <Text style={{ fontSize: 9, fontWeight: '700', color: theme.colors.accent }}>{t('log.mealTag')}</Text>
                            </View>
                          )}
                        </View>
                        <Text style={[styles.itemAmount, { color: theme.colors.textTertiary }]}>{formatMass(item.amount_g, system)}</Text>
                      </View>
                      <View style={{ alignItems: 'flex-end' }}>
                        <Text style={[styles.itemCals, { color: theme.colors.textPrimary }]}>{Math.round(item.calories)}</Text>
                        <View style={styles.itemMacros}>
                          <Text style={{ fontSize: 9, color: theme.colors.protein }}>P{gToDisplayValue(item.protein_g, system).toFixed(massDecimals)}</Text>
                          <Text style={{ fontSize: 9, color: theme.colors.carbs }}>C{gToDisplayValue(item.carbs_g, system).toFixed(massDecimals)}</Text>
                          <Text style={{ fontSize: 9, color: theme.colors.fat }}>F{gToDisplayValue(item.fat_g, system).toFixed(massDecimals)}</Text>
                        </View>
                      </View>
                        ...
                        {item.meal_ingredients!.map((ing, idx) => (
                          <View key={`${ing.food_name}-${idx}`} style={[styles.subIngredient, { backgroundColor: theme.colors.backgroundElevated }]}>
                            <View>
                              ...
                              <Text style={{ fontSize: 10, color: theme.colors.textTertiary }}>{formatMass(ing.amount_g, system)}</Text>
                            </View>
                            <Text style={{ fontSize: 12, color: theme.colors.textTertiary }}>{Math.round(ing.calories)} {t('common.kcal')}</Text>
                          </View>
                        ))}
                      </View>
                    )}
        ...
        <Card style={{ padding: 0, overflow: 'hidden' }}>
          <View style={styles.mealHeader}>
            <View style={styles.mealLeft}>
              ...
              <View>
                <Text style={[styles.mealName, { color: theme.colors.textPrimary }]}>{t('log.exercise')}</Text>
                {totalBurned > 0 && (
                  <Text style={[styles.mealMeta, { color: theme.colors.calories }]}>{t('log.totalBurned', { count: Math.round(totalBurned) })}</Text>
                )}
              </View>
            </View>
            <Pressable onPress={() => setShowExercise(true)} style={[styles.addPill, { backgroundColor: theme.colors.calories + '22' }]}>
              ...
              <Text style={{ fontSize: 12, fontWeight: '600', color: theme.colors.calories }}>{t('common.add')}</Text>
            </Pressable>
          </View>
          {exerciseLogs.map((ex) => (
            <View key={ex.id} style={[styles.itemRow, { borderTopColor: theme.colors.cardBorder }]}>
              <View style={styles.itemMain}>
                <View style={{ flex: 1 }}>
                  ...
                  {ex.duration_min ? <Text style={[styles.itemAmount, { color: theme.colors.textTertiary }]}>{ex.duration_min} {t('common.minutesShort')}</Text> : null}
                </View>
      ...
      <ShareQRModal
        ...
        heading={dateStr}
        meta={[{ label: t('dashboard.itemCount', { count: logs.length }), color: theme.colors.textSecondary }, { label: `${Math.round(totalCals)} ${t('common.kcal')}`, color: theme.colors.accent }]}
        hint={t('log.shareHint')}
        ...
        <View style={styles.overlay}>
          ...
          <SafeAreaView edges={['bottom']} style={[styles.importSheet, { backgroundColor: theme.colors.card, borderTopLeftRadius: theme.style.cardRadius + 6, borderTopRightRadius: theme.style.cardRadius + 6 }]}>
            <View style={styles.importHeader}>
              <View>
                <Text style={[styles.title, { fontSize: 16, color: theme.colors.textPrimary }]}>{t('log.importTitle')}</Text>
                {importingLog && <Text style={{ fontSize: 11, color: theme.colors.textTertiary, marginTop: 2 }}>{t('log.importFrom', { date: importingLog.d })}</Text>}
              </View>
              ...
              {importingLog && MEAL_TYPES.map((mt) => {
                ...
                return (
                  <View key={mt} style={{ marginBottom: 10 }}>
                    <Text style={{ fontSize: 11, color: theme.colors.textTertiary, marginBottom: 4 }}>{t(MEAL_TYPE_LABEL_KEYS[mt])}</Text>
                    {entries.map((entry, i) => (
                      <View key={`${entry.n}-${i}`} style={[styles.subIngredient, { backgroundColor: theme.colors.backgroundElevated, marginBottom: 4 }]}>
                        <View>
                          ...
                          <Text style={{ fontSize: 10, color: theme.colors.textTertiary }}>{formatMass(entry.a, system)}</Text>
                        </View>
                        <Text style={{ fontSize: 12, color: theme.colors.textTertiary }}>{entry.c} {t('common.kcal')}</Text>
                      </View>
                    ))}
                  </View>
                )
            ...
            <Text style={{ fontSize: 11, color: theme.colors.textTertiary, textAlign: 'center', marginTop: 6 }}>
              {dateStr === todayStr ? t('log.willBeAddedToToday') : t('log.willBeAddedToDate', { date: dateStr })}
            </Text>
            <Pressable
            ...
            >
              ...
              <Text style={{ color: theme.colors.onAccent, fontWeight: '700', fontSize: 14 }}>{t('log.addToLog')}</Text>
            </Pressable>
          </SafeAreaView>
        </View>
      </Modal>
      ...
      <PaywallModal
        ...
        headline={t('log.paywallShareHeadline')}
